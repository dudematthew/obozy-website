...
function rehypeRewriteLocalImgs (ctx) {
  return (tree) => {
    ...
    visit(tree, 'element', (node) => {
      ...
      let rel = clean.includes(assetsMarker)
        ? clean.split(assetsMarker).pop()
        : path.basename(clean)
      rel = rel.replace(/\\/g, '/')
      const dest = path.join(ctx.destDir, ...rel.split('/'))
      fs.mkdirSync(path.dirname(dest), { recursive: true })
      fs.copyFileSync(abs, dest)
      const publicPath = `/${ctx.publicPathPrefix}/${rel}`.replace(/\/+/g, '/')
...
async function main () {
  ...
  for (const m of cfg.manuals) {
    ...
    builtMeta.push({
      ...
      image: ir.meta.logoUrl ? `https://obozy.org.pl${ir.meta.logoUrl}` : null,
      imageAlt: ir.meta.title
    })
  }
  ...
  const metaEntries = builtMeta
    ...
    .map((m) => {
      ...
      const image = m.image ? `, image: ${JSON.stringify(m.image)}` : ''
      const imageAlt = `, imageAlt: ${JSON.stringify(m.imageAlt || m.title)}`
      return `  ${JSON.stringify(m.id)}: { title: ${JSON.stringify(title)}, description: ${JSON.stringify(desc)}${image}${imageAlt} }`
    })
    .join(',\n')
  const metaFile = `// Auto-generated by build:manuals — do not edit manually.\nexport const manualsMeta = {\n${metaEntries}\n}\n`
  // Prefixed with _ so Netlify does not treat this data file as an edge function.
  const metaOut = path.join(projectRoot, 'netlify', 'edge-functions', '_manuals-meta.js')
